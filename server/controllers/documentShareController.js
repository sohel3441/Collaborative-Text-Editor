  ...
  // Share a document with another user (email or id)
  ...
  //     const existing = doc.permissions.find((p) => p.user.toString() === userToShare._id.toString());
  //     if (existing) {
  //       existing.role = role;
  //     } else {
  ...
  export const revokeAccess = async (req, res, next) => {
    try {
      ...
      doc.permissions = doc.permissions.filter((p) => p.user.toString() !== userId);
      await doc.save();
  ...
  export const listSharedUsers = async (req, res, next) => {
    try {
      const { id } = req.params;
      const doc = await Document.findById(id).populate('permissions.user', 'name email role');
      if (!doc) return res.status(404).json({ message: 'Document not found' });
